...
\newcommand{\sampletext}{Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore. 12345 \addfontfeatures{Numbers=OldStyle}12345}
...
\noindent Though designed with the needs of medieval scholars in mind,
the Junicode font has proven useful for general users and scholars in
many fields, and it covers many ancient and modern languages. If you
wish to see better support for any language, please leave a request at
the Junicode project page (http://sourceforge.net/projects/junicode).
...
richest in OpenType features. The font implements most of the
recommendation of the Medieval Unicode Font Initiative (version
3.0---compliance with version 4.0 is in progress).  Look for special
MUFI characters (those not in the Unicode standard) in the Private Use
Area (U+E000 and above). Download the complete recommendation at
http://www.mufi.info/.
...
{\greekfont αβγδεζηθικλμνξοπρςστυφχψω\\
...
{\small\itshape The Greek typeface packaged with Junicode is
  FoulisGreek, named for the Foulis brothers, publishers of the famous
  Glasgow Homer (1756--8), which used the Greek Double Pica cut by
  Alexander Wilson. It is not really suitable for
...
When this feature is on (as it is by default in most applications,
though it must be turned on explicitly in Microsoft Word), Junicode
...
not anticipated. In FoulisGreek, a number of alternative letter shapes
can be accessed in this way: e.g. {\greekfont β\salt{β}γ\salt{γ}ρ\salt{ρ}τ\salt{τ}φ\salt{φ}.}

\subsection*{Kerning (kern)}

In most text-based applications, kerning is on by default, but in
Microsoft Word it must be turned on explicitly. Turn kerning off for
an antique look.
...
of historical ligatures in FoulisGreek, e.g.
...
Greek has been moved to the FoulisGreek font, IPA characters based on
...
usual. Use Stylistic Set 10 to access these: a/a, {\addfontfeatures{StyleSet=longslash}a/a}.
...
One character (U+0294) has an alternate shape used in phonetic
...
\noindent This document was set with LuaTeX.
\end{document}
